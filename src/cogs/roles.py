...
from src.config import settings, ResolvedRole
...
class RolesCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.roles: List[ResolvedRole] = []
        self.bot.discord_guild = None

    def build_view(self) -> tuple[discord.ui.View, str]:
        msg_txt_1 = (
        ...
        for r in self.roles:
            ...
            role = self.bot.discord_guild.get_role(int(r.role_id))
    ...
    async def on_ready(self):
        ...
        for role in settings.roles:
            discord_role: discord.Role = discord.utils.find(
                lambda x: x.name == role.name, self.bot.discord_guild.roles
            )
